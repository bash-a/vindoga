...
FocusScope {
    ...
    NavigationBar {
        ...
        Component.onCompleted: {
        	breadcrumbs.addCrumb(qsTr("Launcher"), launchview, "", "../ico/16/vindoga-market.png")
//         	breadcrumbs.addCrumb(qsTr("Packages"), listview, "", "../ico/16/vindoga-market.png")
//         	breadcrumbs.addCrumb(qsTr("Packages"), catview, "", "../ico/16/vindoga-market.png")
        }

        onCrumbClicked: {
            if (index < 1 ||
                (index == 2 && navigation.breadcrumbs.model.get(2).key == categoryKey)) {
                searchQuery = ""
        ...
        searchBoxVisible: false // switcher.currentFrame() != detailsview

        function doSearch() {
            if (searchQuery.length > 0) {
                var bc = navigation.breadcrumbs
                if (bc.count < 2 ||
                    (bc.count == 3 && bc.model.get(2).key == categoryKey)) {
    ...
    Frame {
        ...
        LauncherView {
            ...
            focus: true
            onLaunchApp: {
                launchermodel.launchApp(appname)
